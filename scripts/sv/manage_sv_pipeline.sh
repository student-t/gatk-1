...
while true; do
    ...
    case $yn in
        [Yy]*)  SECONDS=0
                echo "GATK_SV_TOOL=${GATK_SV_TOOL} runWholePipeline.sh ${GATK_DIR} ${PROJECT_NAME} ${CLUSTER_NAME} ${OUTPUT_DIR} ${CLUSTER_BAM} ${CLUSTER_REFERENCE_2BIT} ${CLUSTER_REFERENCE_IMAGE} ${COPY_FASTQ} ${SV_ARGS} 2>&1 | tee -a ${LOCAL_LOG_FILE}" | tee -a ${LOCAL_LOG_FILE}
                GATK_SV_TOOL=${GATK_SV_TOOL} runWholePipeline.sh ${GATK_DIR} ${PROJECT_NAME} ${CLUSTER_NAME} ${OUTPUT_DIR} ${CLUSTER_BAM} ${CLUSTER_REFERENCE_2BIT} ${CLUSTER_REFERENCE_IMAGE} ${COPY_FASTQ} ${SV_ARGS} 2>&1 | tee -a ${LOCAL_LOG_FILE}
